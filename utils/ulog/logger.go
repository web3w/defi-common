...
func unmarshalLevel(lvl string) zapcore.Level {
	...
	if lvl != "" {
		if err := level.UnmarshalText([]byte(lvl)); err != nil {
			fmt.Fprintln(os.Stderr, fmt.Sprintf("Invalid log level %s. Fall back to use Info level.", lvl))
...
func newProductionLogger(level string) *zap.SugaredLogger {
	...
	highPriorityEnabler := zap.LevelEnablerFunc(func(lvl zapcore.Level) bool {
	...
	if pkgCfg.Name != "" {
		...
		configForFile.EncodeCaller = EncodeCallerWithFuncName
		fileOut := zapcore.AddSync(newRotatedLogger(pkgCfg.LogFile(), &pkgCfg.Rotation))
		fileErrOut := zapcore.AddSync(newRotatedLogger(pkgCfg.ErrorLogFile(), &pkgCfg.Rotation))
		fileEncoder := zapcore.NewJSONEncoder(configForFile)
		cores = append(cores, zapcore.NewCore(fileEncoder, fileOut, normalEnabler))
		cores = append(cores, zapcore.NewCore(fileEncoder, fileErrOut, highPriorityEnabler))
	}

	if pkgCfg.Console {
		configForConsole := config
		configForConsole.EncodeCaller = EncodeCaller
		consoleOut := zapcore.Lock(os.Stderr)
		consoleEncoder := zapcore.NewConsoleEncoder(configForConsole)
		cores = append(cores, zapcore.NewCore(consoleEncoder, consoleOut, normalEnabler))
	}
...
func (l *ulogger) V(level int) bool {
	return unmarshalLevel(l.level) >= unmarshalLevel("info")
}
